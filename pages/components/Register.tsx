...
const Register: React.FC<Props> = ({ email }) => {
  ...
  const isInvalidPassword = (password: string) => { // safe password regex - not currently used
    const safePasswordRegex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d@$!%*#?&^_-]{8,}$/;
    return !safePasswordRegex.test(password);
  };

  const areInputsInvalid = !password || !confirmPass || isPasswordInvalid;
  ...
  return (
    <div>
      <h1 className="text-md font-bold mt-6">{email}</h1>
      <div className="w-full flex flex-col self-center items-center">
